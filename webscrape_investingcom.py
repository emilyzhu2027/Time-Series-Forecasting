...
def getPayload(startDate, endDate, dataFreq, commodityName):
    ...
    payload = 'curr_id=' + str(commodityIDNums[commodityName]) + \
            '&st_date=' + startDate + \
            '&end_date=' + endDate + \
...
def getCommoditySoup(startDate, endDate, dataFreq, commodityName, headers):
    # User agent and header declaration so I won't get classified as spam :)
...
def getCommodityData(soup_data, commodityName):
    ...
    oneCommDataDf = pd.concat(listPriceSeries, axis = 0)
